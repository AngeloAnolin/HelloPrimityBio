...
export default function (window) {
  ...
  // Helper vars and functions.
	...
	TabsNav.prototype._openTab = function (tab) {
		...
		var currentDimensions = { left: bounds.left, top: bounds.top, width: bounds.width, height: bounds.height }
		var self = this
		...
		var animeBars = { targets: this.options.movable === 'all' ? this.DOM.bars : this.DOM.bars[this.current] }
		var animeTabsDelay = function (target, index, cnt) {
		...
		var animeTabsTranslation = function (target, index, cnt) {
		...
		var animeBarsScale = function (target, index, cnt) {
		...
		if (this.extraEl) {
			...
			var extraBounds = this.extraEl.getBoundingClientRect()
			var animeExtraTranslation = this.options.layout === 'vertical' ? this.dim.win - (this.dim.position + this.dim.measure) + Math.abs(extraBounds.left - this.dim.position) + this.dim.measure : -1 * this.dim.position
	...
	TabsNav.prototype.show = function (callback) {
		...
		this.DOM.tabs.forEach(function (tab) {
			...
			var title = tab.querySelector('.tabsnav__title')
	...
	TabsNav.prototype.toggleVisibility = function () {
		...
		var self = this
		var endAnimation = function () { self.isAnimating = false }
	...
	TabsNav.prototype._resize = function () {
		...
		if (this.isOpen) {
			...
			var translateStr = this.options.layout === 'vertical' ? 'translateX' : 'translateY'
			var scaleStr = this.options.layout === 'vertical' ? 'scaleX' : 'scaleY'
